...
def render_html_for_run(run_json_path: str, out_html_path: str=None):
    ...
    for country, queries in data.get("matches", {}).items():
        html_lines.append(f"<h2>{country}</h2>")
        for query, jobs in queries.items():
            html_lines.append(f"<h3>Query: {query} ({len(jobs)})</h3><div style='display:flex;flex-wrap:wrap'>")
            for j in jobs:
